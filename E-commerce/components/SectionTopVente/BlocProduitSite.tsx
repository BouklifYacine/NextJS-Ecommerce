  ...
  return (
    ...
    {data?.map((d) => (
      <div key={d.id} className="w-full md:w-[45%] lg:w-[30%]">
        <BlocUnique nom={d.nom} image={d.images[0].urlImage} prix={d.prix} prixpromo={d.prixPromo || null} categorie={d.categorie} description={d.description} stock={d.quantiteStock} />
      </div>
    ))}
  </div>
  );
